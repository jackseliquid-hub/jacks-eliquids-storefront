...
export default function AddProductPage() {
  ...
  useEffect(() => {
    async function load() {
      const [cats, tags, brands] = await Promise.all([
        getCategories(),
        getTags(),
        getBrands(),
      ]);
  ...
  function setField<K extends keyof Product>(key: K, value: Product[K]) {
    setProduct(prev => {
        ...
        // Auto-slugify the name if the slug hasn't been manually tampered with
  ...
  // ── Attributes & Variations Handlers ──────────────────────────────────────
  ...
  function updateAttributeValues(key: string, commaString: string) {
    setProduct(prev => {
      ...
      return {
        ...prev,
        attributes: { ...prev.attributes, [key]: vals }
      };
    });
  }
  ...
  function generateVariations() {
    ...
    showToast(`Generated ${newVariations.length} variations!`);
  }
  ...
  function applyPriceToAll() {
    ...
    const formatted = bulkPrice.startsWith('£') ? bulkPrice : `£${Number(bulkPrice).toFixed(2)}`;
    setProduct(prev => ({
      ...prev,
  ...
  function applyStockToAll() {
    ...
    showToast(`Stock status applied to all variations`);
  }
      ...
      <div className={styles.pageHeader}>
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
            <Link href="/admin" className={styles.storeFrontLink}>← Cancel</Link>
          </div>
          <h1 className={styles.pageTitle} style={{ marginTop: '0.35rem' }}>Add New Product</h1>
        </div>
            ...
            <div className={styles.formGrid}>
              <div className={styles.formGroup} style={{ gridColumn: '1 / -1' }}>
                ...
                <input
                  ...
                  value={product.name}
                  placeholder="e.g. Cherry Ice Nic Salt"
            ...
            {product.variations.length > 0 && (
              <div style={{ borderTop: '1px solid #eaeaea', paddingTop: '1.5rem' }}>
                 <div className={styles.bulkBar} style={{ marginBottom: '1rem' }}>
                    ...
                    />
                    <button type="button" className={`${styles.btn} ${styles.btnSecondary}`} style={{ padding: '0.4rem 0.9rem', fontSize: '0.8rem' }} onClick={applyPriceToAll}>
                      Apply Price
                    </button>

                    <select className={styles.bulkInput} value={bulkStock} onChange={e => setBulkStock(e.target.value)} style={{ width: 'auto', marginLeft: '1rem' }}>
                      <option value="">Stock Status...</option>
                      <option value="true">In Stock</option>
                      <option value="false">Out of Stock</option>
                    </select>
                    <button type="button" className={`${styles.btn} ${styles.btnSecondary}`} style={{ padding: '0.4rem 0.9rem', fontSize: '0.8rem' }} onClick={applyStockToAll}>
                      Apply Stock
                    </button>
                </div>
                    ...
                    <tbody>
                      {product.variations.map((v, i) => (
                        <tr key={v.id}>
                          ...
                          <td>
                             <button type="button" onClick={() => setProduct(prev => ({...prev, variations: prev.variations.filter((_, idx) => idx !== i)}))} style={{ color: 'red', background: 'none', border: 'none', cursor: 'pointer' }}>✕</button>
                          </td>
                        </tr>
                      ))}
                    </tbody>
