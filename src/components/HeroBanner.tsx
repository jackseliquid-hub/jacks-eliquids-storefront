...
function Slide({ slide, state }: { slide: Banner; state: 'enter' | 'active' | 'exit' }) {
  const isLight = slide.text_color === 'light';

  // CSS transform for smooth horizontal slide
  const transform = state === 'enter'
    ? 'translateX(100%)'
    : state === 'exit'
      ? 'translateX(-30%)'
      : 'translateX(0)';
  const opacity = state === 'exit' ? 0 : 1;
  const zIndex = state === 'active' || state === 'enter' ? 2 : 1;
...
export default function HeroBanner({ banners }: Props) {
  ...
  const goTo = useCallback((idx: number) => {
    ...
    setPrevious(current);
    setCurrent(idx);
    // Clear the exiting slide after the transition completes
  ...
  useEffect(() => {
    if (active.length <= 1 || isPaused || previous !== null) return;
    timerRef.current = setTimeout(next, 5000);
    return () => { if (timerRef.current) clearTimeout(timerRef.current); };
  }, [current, isPaused, next, active.length, previous]);
  ...
  return (
    ...
    >
      ...
      >
        {/* Exiting (previous) slide */}
        {previous !== null && (
          <Slide slide={active[previous]} state="exit" />
        )}

        {/* Current (entering then active) slide */}
        <Slide
          slide={active[current]}
          state={previous !== null ? 'enter' : 'active'}
        />
        ...
        {/* Progress bar */}
        {active.length > 1 && !isPaused && previous === null && (
          <div style={{
